...
if ( !class_exists( 'ZP_Session' ) ) {
    final class ZP_Session extends Recursive_ArrayAccess implements Iterator, Countable {
        ...
        protected function set_cookie(){
            if( !headers_sent() )
                setcookie(ZP_SESSION_COOKIE,$this->session_id.'||'.$this->expires.'||'.$this->exp_variant,$this->expires,COOKIEPATH,COOKIE_DOMAIN );
        }
    ...
    function ZPStd_session_cleanup() {
        ...
        if ( ! defined( 'ZP_INSTALLING' ) ) {
            $expiration_keys = $wpdb->get_results( "SELECT option_name, option_value FROM $wpdb->options WHERE option_name LIKE '_ZPStd_session_expires_%'" );
            $now = time();
